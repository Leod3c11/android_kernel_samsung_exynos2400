...
static struct kobject *sram_export_kobj;
...
static int margin_cpucl1l;
static int margin_cpucl1h;
...
static int margin_unpu;
static int margin_icpu;
static int margin_chubvts;
...
module_param(margin_cpucl1l, int, 0);
module_param(margin_cpucl1h, int, 0);
...
module_param(margin_unpu, int, 0);
module_param(margin_icpu, int, 0);
module_param(margin_chubvts, int, 0);
...
void margin_table_init(void)
{
	...
	init_margin_table[MARGIN_CPUCL1L] = margin_cpucl1l;
	init_margin_table[MARGIN_CPUCL1H] = margin_cpucl1h;
	...
	init_margin_table[MARGIN_ICPU] = margin_icpu;
	...
	init_margin_table[MARGIN_UNPU] = margin_unpu;
	init_margin_table[MARGIN_CHUBVTS] = margin_chubvts;
...
static int get_vclk_id_from_margin_id(int margin_id)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (vclk && vclk->margin_id == margin_id)
			return i;
	}
...
attr_percent(MARGIN_CPUCL1L, cpucl1l_margin);
attr_percent(MARGIN_CPUCL1H, cpucl1h_margin);
...
static struct attribute *percent_margin_attrs[] = {
	...
	&cpucl1l_margin_percent.attr,
	&cpucl1h_margin_percent.attr,
...
static const char *fvmap_domain_name(unsigned int idx)
{
...
static int fvmap_margin_id(unsigned int idx)
{
	struct vclk *vclk = cmucal_get_node(ACPM_VCLK_TYPE | idx);

	if (vclk)
		return vclk->margin_id;

	return -1;
}

static ssize_t fvmap_build_sram_text_dump(char *buf, size_t buf_size)
{
	struct fvmap_header *fvmap_header;
	struct rate_volt_header *fv_table;
	ssize_t len = 0;
	int size, i, j;
	int margin_id, current_margin;

	if (!buf || !sram_fvmap_base)
		return -EINVAL;
	...
	for (i = 0; i < size && len < buf_size; i++) {
		if (!fvmap_header[i].num_of_lv)
			continue;

		fv_table = sram_fvmap_base + fvmap_header[i].o_ratevolt;
		margin_id = fvmap_margin_id(i);
		current_margin = (margin_id >= 0 && margin_id < MAX_MARGIN_ID) ?
			percent_margin_table[margin_id] : 0;

		len += scnprintf(buf + len, buf_size - len,
				 "Domain: %s (ID: 0x%x, Margin ID: %d)\n",
				 fvmap_domain_name(i), i, margin_id);
		len += scnprintf(buf + len, buf_size - len,
				 "Levels: %d, Members: %d\n",
				 fvmap_header[i].num_of_lv,
				 fvmap_header[i].num_of_members);
		...
		for (j = 0; j < fvmap_header[i].num_of_lv && len < buf_size; j++) {
			len += scnprintf(buf + len, buf_size - len,
					 "%-5d | %-14u | %-13u |\n",
					 j,
...
static ssize_t sram_data_read(struct file *filp, struct kobject *kobj,
			      struct bin_attribute *attr, char *buf,
			      loff_t off, size_t count)
{
	char *dump_buf;
	ssize_t dump_len;
	size_t dump_capacity = PAGE_SIZE * 32;

	if (!sram_fvmap_base)
		return -ENODEV;

	dump_buf = vzalloc(dump_capacity);
	if (!dump_buf)
		return -ENOMEM;

	dump_len = fvmap_build_sram_text_dump(dump_buf, dump_capacity);
	if (dump_len < 0) {
		vfree(dump_buf);
		return dump_len;
	}
	...
	if (count > dump_len - off)
		count = dump_len - off;

	memcpy(buf, dump_buf + off, count);
	vfree(dump_buf);
...
static int fvmap_create_sram_export_sysfs(void)
{
	int ret;

	sram_export_kobj = kobject_create_and_add("sram_export", kernel_kobj);
	if (!sram_export_kobj)
		return -ENOMEM;

	ret = sysfs_create_bin_file(sram_export_kobj, &sram_data_attr);
	if (ret) {
		kobject_put(sram_export_kobj);
		sram_export_kobj = NULL;
		return ret;
	}
...
int fvmap_init(void __iomem *sram_base)
{
	...
	if (fvmap_create_sram_export_sysfs())
		pr_err("Fail to create sram_export sysfs\n");
